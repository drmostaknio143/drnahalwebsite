# Dr. Nahal Mostak Khan Arnob — Website Developer Handoff Brief

**Project:** Bilingual (English + Bengali) website for Dr. Nahal Mostak Khan Arnob, Consultant Vitreoretinal, Cataract & Refractive Surgeon, Assistant Professor at Enam Medical College.

**Date of this brief:** 2026-09-07
**Prepared for:** Handoff to development team (human developer + their AI coding assistant)
**Current repo folder name:** `dr-nahal-website/`

This document is the single source of truth for "what exists, what's locked, what's left, and how to build the rest so it matches what's already there." Read this entire document before writing any code. Where this brief conflicts with your own instincts about "best practice," this brief wins — the client has made explicit, non-negotiable decisions documented below.
...
## 4. What Is Built Today (as of 2026-09-07)
...
- **Only 20 of the doctor's ~91 Facebook Reels have been collected so far.** The remaining ~71 are being collected manually by the client (Facebook rate-limits/login-walls automated bulk collection past the first ~20). See `8. Facebook Reels Inventory/reels-inventory.csv` for the tracking sheet (columns: Position, ViewCount, URL, Category, UsedOnWebsite, Notes) — **append new rows here as more reels are collected, don't create a separate list.**
...
**Everything built so far is fully bilingual** — every page, every component, every button, every piece of copy that exists in code today has both an `en` and `bn` version, and both have been visually verified in the browser (desktop + mobile) by the previous session. The language switcher (EN/BN pill in the navbar) preserves the current path when switching. **There is no partially-translated content live on the site right now** — if a page/section doesn't exist yet, it doesn't exist in *either* language; nothing is "English-only."

### 4.9 Website copy — what's written vs. what's implemented
**This is an important distinction: copywriting and implementation are two separate, independently-tracked things.** All copy was written up front, in a separate planning phase, before most of the actual coding started. Location: `4. Website Copy Plan/` folder, sibling to this brief.

| Copy file | Content | Written (EN+BN)? | Implemented in code? |
|---|---|---|---|
| `01 - Home & About` | Home page copy, About page copy incl. career roadmap milestones | ✅ Yes | ✅ Yes |
| `02 - Services` | All 11 services' full copy (intro, who-needs-it, how-it-works, notes, CTAs) | ✅ Yes | ✅ Yes |
| `03 - Conditions - Part 1 & 2` | 16 conditions/diseases, full copy | ✅ Yes | ❌ **No page built yet** |
| `04 - Video Library, Gallery, Chambers, Contact` | Page-level copy for these 4 sections | ✅ Yes | ⚠️ Partial — Video Library and Gallery exist as *home page teaser sections* only, not as their own dedicated pages; Chambers and Contact have no dedicated page at all yet |
| `05 - FAQ, Privacy, Terms, 404` | Full copy for these 4 pages | ✅ Yes | ❌ **No pages built yet** |
| `06-10 - Blog Posts 1-10` | 10 full SEO blog posts | ✅ Yes | ❌ **No blog system/pages built yet** |

**In short: the copywriting for the entire site is 100% done. Implementation is roughly 30-35% done** (Home, About, Services). Everything else in the table above just needs to be turned into working pages using copy that already exists, word-for-word — **do not write new copy for these pages; use what's in the `4. Website Copy Plan/` files.** If anything there is ambiguous or looks incomplete, ask the client rather than inventing new copy.
...
2. **Conditions page** (`/conditions`) — 16 conditions, copy already written (`03 - Conditions...md`, Parts 1 & 2). The client has not yet confirmed the exact interaction pattern for this page — **ask the client whether to use the same pinned-scroll-card style as Services, or a simpler grid+modal layout**, before building. Do not assume.
...
6. **FAQ page** (`/faq`) — copy ready in `05 - FAQ...md`.
7. **Privacy Policy page** (`/privacy`) — copy ready.
8. **Terms of Use page** (`/terms`) — copy ready.
9. **Custom 404 page** — copy ready.
10. **Blog system** — 10 SEO posts already written (`06-10 - Blog Posts...md`). Needs: a blog index page, individual post pages, and a repeatable pattern for adding more posts later (see Section 9, SEO content pattern).
...
Three raw, unedited photos of Dr. Nahal exist at:
`1. Doctor's Information/Images Provided by Dr. Nahal/Dr. Nahal Image 1.jpeg` (and `2.jpeg`, `3.jpeg`)
...
- **Conditions** (currently 16, in `conditions.ts` — this file is a bare-bones stub today, needs full bilingual content matching what's already written in `03 - Conditions...md`)
...
Ten SEO blog posts already exist (`06-10 - Blog Posts 1-10 (EN+BN).md`) as the established pattern for any future content. Before writing new pages/posts/copy, read a couple of these existing posts and match:
...
Location: `1. Doctor's Information/Images Provided by Dr. Nahal/` — 3 raw photos.
...
Client planning folder (this document's sibling folders):
```
14. Dr. Nahal _Opthalmologists/
├── 1. Doctor's Information/            → CV, GMC cert, banner, visiting card, doctor's 3 raw photos
├── 2. Design Guidelines/               → locked color/font decisions history
├── 3. REFERENCE ONLY - Mock HTML/      → early animation prototype, superseded by real Next.js code
├── 4. Website Copy Plan/               → 📝 ALL site copy, 100% written, EN+BN (see Section 4.9 table)
├── 5. For Home_Hero section.../        → the real AI-generated eye images used in the locked hero
├── 6. Frontend Master Spec/            → earlier planning doc, largely superseded by this brief
├── 7. Service List Based Images/       → real photos used on the Services page
├── 8. Facebook Reels Inventory/        → reels-inventory.csv, append new reel URLs here
└── 9. Developer Handoff Brief/         → this document
```
...
- Write new copy for pages that already have copy written in `4. Website Copy Plan/`
